<template>
	<div>
		<v-card>
			<v-app-bar color="rgba(255,255,255,0.7)" fixed>
				<v-app-bar-nav-icon
				...
				<v-spacer></v-spacer>
				<v-btn icon @click="openPostWin" v-if="isLogin"
					><v-icon>mdi-pencil-plus-outline</v-icon></v-btn
				>
				<v-menu>
					...
					<v-list>
						<v-list-item @click="switchPartical">
							<v-list-item-title> 粒子背景开关 </v-list-item-title>
						</v-list-item>
					</v-list>
				</v-menu>
			</v-app-bar>
		...
		>
			<v-list>
				<v-btn block text large @click="jumpto('mine')" v-if="isLogin">
					...
					<v-list-item-content> 我的 </v-list-item-content>
				</v-btn>
				...
				<v-btn v-else block text large @click="loginDialog = !loginDialog">
					...
					<v-list-item-content> 登录/注册 </v-list-item-content>
				</v-btn>
				...
				<v-btn block text large @click="jumpto('message')">
					...
					<v-list-item-content> 消息 </v-list-item-content>
				</v-btn>
				...
				<v-btn block text large @click="jumpto('group')">
					...
					<v-list-item-content> 群组 </v-list-item-content>
				</v-btn>
				...
				<v-btn block text large @click="jumpto('topic')">
					...
					<v-list-item-content> 话题 </v-list-item-content>
				</v-btn>
...
export default {
	...
	methods: {
		...
		getGroups() {
			this.axios.get("getGroupNames").then((res) => {
				if (res.status == 200) {
