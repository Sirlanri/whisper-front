<template>
	<v-col cols="12" sm="6" offset-sm="0">
		...
		<v-dialog v-model="replyWin" max-width="500">
			<v-card>
				<v-card-title> 回复消息 </v-card-title>
				<v-card-text>
					<v-textarea
						label="输入你的回复"
						outlined
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn outlined large color="error" @click="replyWin = false"
						>取消</v-btn
					>
					<v-btn color="primary" large @click="sendReply" :disabled="btndis"
						>发送</v-btn
					>
				</v-card-actions>
			</v-card>
		...
		<v-dialog v-model="groupDelDialog" max-width="500">
			<v-card>
				...
				<v-card-text> 将会删除这条回复 </v-card-text>
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn outlined large @click="groupDelDialog = false">取消</v-btn>
					<v-btn color="error" large @click="delReply" :disabled="btndis"
						>删除</v-btn
					>
				</v-card-actions>
			</v-card>
