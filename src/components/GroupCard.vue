<template>
	<v-col lg="4" sm="6" cols="12" offset-sm="0">
		<v-card class="group-card" v-ripple @click="openPost">
			<v-img class="align-start justify-end" height="200px" :src="imgsrc">
				<v-menu v-if="isAdmin">
			...
			<v-card-title>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on, attrs }">
						<span class="count-num" v-bind="attrs" v-on="on"
							>&nbsp;{{ amount }}</span
						>
					</template>
					<span>此群的文章数</span>
		...
		<v-dialog v-model="delDialog" max-width="400">
			<v-card>
				<v-col cols="10" offset="1">
					<v-card-title> 确认删除此群？ </v-card-title>
					<v-card-text>
						群信息会被删除，群内的post会被修改为无群状态
					</v-card-text>
					<v-card-actions class="justify-center">
						<v-btn @click="delDialog = false" large color="primary">取消</v-btn>
						<v-btn @click="delGroupOnly" large color="red" :disabled="btndis"
							>删除</v-btn
						>
					</v-card-actions>
				</v-col>
		...
		<v-dialog v-model="delAllDialog" max-width="400">
			<v-card>
				<v-col cols="10" offset="1">
					...
					<v-card-text> 此操作会删除群及其群里的全部post！ </v-card-text>
					<v-card-actions class="justify-center">
						<v-btn @click="delAllDialog = false" large color="primary"
							>取消</v-btn
						>
						<v-btn @click="delGroupAll" large color="red" :disabled="btndis"
							>删除</v-btn
						>
					</v-card-actions>
				</v-col>
